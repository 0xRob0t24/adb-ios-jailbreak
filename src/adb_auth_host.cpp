...
static bool write_public_keyfile(RSA* private_key, const std::string& private_key_path) {
    ...
    // แทนที่ EVP_EncodedLength ด้วยการคำนวณเอง
    size_t expected_length = 4 * ((sizeof(binary_key_data) + 2) / 3); 

    std::string content;
    content.resize(expected_length);
    size_t actual_length = EVP_EncodeBlock(reinterpret_cast<uint8_t*>(&content[0]), 
                                           binary_key_data, sizeof(binary_key_data));
    content.resize(actual_length);  // ปรับขนาดตามค่าที่เข้ารหัสจริง
