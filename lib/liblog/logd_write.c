...
*/
#include <stdio.h>  // เพิ่มการนำเข้าไลบรารีนี้
#include <stdarg.h> // สำหรับการใช้ va_list ใน vsnprintf
#include <string.h> // เพิ่มเพื่อใช้ฟังก์ชันต่าง ๆ เช่น memset หรืออื่น ๆ ถ้าจำเป็น
...
#include <errno.h>
    ...
    for (i = 0; i < LOG_ID_MAX; i++) {
        ...
        const char *log_name = android_log_id_to_name(i);
        
        // คัดลอก "/dev/log_" ลงใน buf
        strncpy(buf, "/dev/log_", sizeof(buf) - 1);
        buf[sizeof(buf) - 1] = '\0'; // ทำให้แน่ใจว่ามี null terminator
        
        // เพิ่ม log_name ลงใน buf
        strncat(buf, log_name, sizeof(buf) - strlen(buf) - 1);
                                              ...
                                              const char *tag, const char *msg)
{
    ...
    if ((bufID != LOG_ID_RADIO) &&
        ...
        !strcmp(tag, "SMS"))) {
            ...
            // ใช้ strncpy และ strncat แทน snprintf
            const char *prefix = "use-Rlog/RLOG-";
            size_t prefix_len = strlen(prefix);
            strncpy(tmp_tag, prefix, sizeof(tmp_tag) - 1); // คัดลอก prefix ลง tmp_tag
            tmp_tag[sizeof(tmp_tag) - 1] = '\0'; // ทำให้แน่ใจว่ามี null terminator

            strncat(tmp_tag, tag, sizeof(tmp_tag) - prefix_len - 1); // เพิ่ม tag ลงใน tmp_tag
            tag = tmp_tag; // อัปเดตค่า tag
    }
...
#define LOG_BUF_SIZE 1024  // ปรับขนาดตามที่ต้องการ

// ฟังก์ชันที่ใช้สำหรับจัดรูปแบบสตริงโดยไม่ใช้ snprintf หรือ vsnprintf
LIBLOG_ABI_PUBLIC int my_simple_format(char *buf, size_t size, const char *fmt, ...) {
    if (size == 0) {
        return 0; // ถ้าขนาดเป็น 0 ก็ไม่ต้องทำอะไร
    }

    va_list args;
    va_start(args, fmt);
    
    // ตัวแปรสำหรับการจัดการสตริง
    int len = 0;
    const char *p;
    char *b = buf;

    // วน loop ผ่าน fmt และจัดการกับตัวแปร
    for (p = fmt; *p != '\0'; p++) {
        if (*p == '%' && *(p + 1) == 's') { // ถ้าเจอ %s
            p++; // ข้าม %
            ...
            if (len < size - 1) { // ตรวจสอบขนาด
                *b++ = *p;
                len++;
            }
        }
    }
    *b = '\0'; // ทำให้เป็น string

    va_end(args);
    
    return len; // คืนค่าจำนวนตัวอักษรที่เขียนได้
}

// ใช้งานในฟังก์ชัน log
LIBLOG_ABI_PUBLIC int __android_log_vprint(int prio, const char *tag,
                                           const char *fmt, va_list ap)
{
    char buf[LOG_BUF_SIZE];

    // ใช้ my_simple_format แทน
    my_simple_format(buf, LOG_BUF_SIZE, fmt, ap);
                                          ...
                                          const char *fmt, ...)
{
    ...
    va_start(ap, fmt);
    my_simple_format(buf, LOG_BUF_SIZE, fmt, ap); // เปลี่ยนเป็น my_simple_format
    va_end(ap);
        ...
        const char *fmt, ...)
{
    ...
    if (fmt) {
        ...
        if (cond)
            my_simple_format(buf, LOG_BUF_SIZE, "Assertion failed: %s", cond); // ใช้ my_simple_format แทน
        else
